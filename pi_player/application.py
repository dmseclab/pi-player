...
_SETUP_ALLOWED=("/setup","/api/setup/","/api/health","/api/player/","/player","/static/","/favicon.ico")

@app.middleware("http")
async def enforce_first_time_setup(request:Request,call_next):
    path=request.url.path
    with db() as conn: setup_required=get_setting(conn,"setup_required","0")=="1"
    if setup_required:
        allowed=path=="/setup" or any(path.startswith(prefix) for prefix in _SETUP_ALLOWED)
        if not allowed:
            if path.startswith("/api/"): return RedirectResponse(url="/setup",status_code=307)
