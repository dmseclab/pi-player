(() => {
  const input=document.querySelector('#asset-file'),form=document.querySelector('#upload-form');
  function isVideoFile(){return !!input?.files?.[0] && /\.(mp4|webm)$/i.test(input.files[0].name);}
  function syncUpload(){if(!form||!input)return;const video=isVideoFile();form.querySelector('h2').textContent='Upload Image or Video';const fileLabel=form.querySelector('label[for="asset-file"]');if(fileLabel)fileLabel.textContent='Image or video file';input.accept='image/*,video/mp4,video/webm';let box=form.querySelector('.video-upload-options');if(!box){box=document.createElement('div');box.className='video-upload-options grid two hidden';box.innerHTML='<label><input type="checkbox" data-upload-video-muted checked> Muted</label><label><input type="checkbox" data-upload-video-loop> Loop video</label>';form.querySelector('.asset-actions')?.insertAdjacentElement('beforebegin',box);}box.classList.toggle('hidden',!video);}
  input?.addEventListener('change',syncUpload);form?.addEventListener('reset',()=>setTimeout(syncUpload,0));form?.addEventListener('formdata',e=>{if(!isVideoFile())return;e.formData.set('video_muted',String(!!form.querySelector('[data-upload-video-muted]')?.checked));e.formData.set('video_loop',String(!!form.querySelector('[data-upload-video-loop]')?.checked));});
