#!/usr/bin/env bash
set -euo pipefail
APP_DIR="/opt/pi-player-rk"; DATA_DIR="/var/lib/pi-player"; LOG_DIR="/var/log/pi-player"; APPLIANCE="${PI_PLAYER_APPLIANCE:-0}"; APP_USER="${PI_PLAYER_USER:-pi}"; [[ "$APPLIANCE" == "1" ]] && APP_USER="pi-player"
...
apt-get update; apt-get install -y git ca-certificates curl python3 python3-venv python3-pip logrotate rsync chromium labwc seatd dbus-user-session xwayland openssh-server sudo network-manager
...
install -m 0644 "$APP_DIR/deploy/pi-player-watchdog.service" /etc/systemd/system/pi-player-watchdog.service; install -m 0644 "$APP_DIR/deploy/pi-player-watchdog.timer" /etc/systemd/system/pi-player-watchdog.timer; chmod 0755 "$APP_DIR/deploy/pi-player-watchdog.sh"
...
if [[ "$APPLIANCE" == "1" ]]; then
 hostnamectl set-hostname pi-player; mkdir -p /etc/systemd/system/getty@tty1.service.d; printf '[Service]\nExecStart=\nExecStart=-/sbin/agetty --autologin %s --noclear %%I $TERM\n' "$APP_USER" > /etc/systemd/system/getty@tty1.service.d/autologin.conf
...
echo "Installed Pi Player RK${APPLIANCE:+ appliance}."; echo "API: http://<pi-address>:8000/"; echo "Health: http://127.0.0.1:8000/api/health"; echo "Reboot to test the full kiosk boot."
